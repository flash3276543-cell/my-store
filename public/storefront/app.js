...
function renderSheet(product) {
  sheetContent.innerHTML = `
    ...
    <p id="orderStatus" class="status" role="status"></p>
  ...
  document.querySelector('#placeOrderButton').addEventListener('click', async (event) => {
    const button = event.currentTarget;
    const orderStatus = document.querySelector('#orderStatus');
    button.disabled = true;
    setStatus(orderStatus, 'Placing order...');
    try {
      await api('/api/orders', { method: 'POST', body: JSON.stringify({ productId: product.id }) });
      setStatus(orderStatus, 'Order placed. Track it from My Account \u2192 My Orders.', 'success');
    } catch (error) {
...
function orderCardMarkup(order) {
  return `<article class="license-card glass">
    ...
    <p>Payment: ${escapeAttribute(capitalize(order.payment_status))} \u2014 Order: ${escapeAttribute(capitalize(order.order_status))}</p>
  </article>`;
}
