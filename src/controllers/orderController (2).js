const orderService = require('../services/orderService');

/** POST /api/orders — a logged-in customer creates an order for one product. */
async function create(req, res, next) {
  try {
    const { productId, paymentMethod } = req.body;
    const order = await orderService.createOrder({
      userId: req.user.sub,
      customerEmail: req.user.email,
      productId,
      paymentMethod: paymentMethod || null,
    });
    res.status(201).json({ data: order });
  } catch (err) {
    next(err);
  }
}

/** GET /api/orders/mine — the logged-in customer's own orders (account page). */
async function mine(req, res, next) {
  try {
    const orders = await orderService.getOrdersForCustomer({ userId: req.user.sub, email: req.user.email });
    res.json({ data: orders });
  } catch (err) {
    next(err);
  }
}

/** GET /api/admin/orders — every order, for the admin panel. View-only for this stage. */
async function adminList(req, res, next) {
  try {
    const orders = await orderService.getAllOrdersForAdmin();
    res.json({ data: orders });
  } catch (err) {
    next(err);
  }
}

/** POST /api/orders/:id/payment-proof — customer's "I HAVE PAID" for a CCP order. */
async function submitProof(req, res, next) {
  try {
    const { providerRef, paymentDate, note } = req.body;
    const result = await orderService.submitPaymentProof({
      orderId: req.params.id,
      userId: req.user.sub,
      email: req.user.email,
      providerRef: providerRef || null,
      paymentDate: paymentDate || null,
      note: note || null,
    });
    res.status(201).json({ data: result });
  } catch (err) {
    next(err);
  }
}

/** POST /api/admin/orders/:id/verify — admin verifies a pending payment (any method); triggers license generation. */
async function adminVerify(req, res, next) {
  try {
    const result = await orderService.verifyOrder(req.params.id);
    res.json({ data: result });
  } catch (err) {
    next(err);
  }
}

/** POST /api/admin/orders/:id/reject — admin rejects a pending payment (any method). */
async function adminReject(req, res, next) {
  try {
    const result = await orderService.rejectOrder(req.params.id);
    res.json({ data: result });
  } catch (err) {
    next(err);
  }
}

/** POST /api/admin/orders/:id/cancel — cancels an order in any state (e.g. an already-completed one). */
async function adminCancel(req, res, next) {
  try {
    const result = await orderService.cancelOrder(req.params.id);
    res.json({ data: result });
  } catch (err) {
    next(err);
  }
}

module.exports = { create, mine, adminList, submitProof, adminVerify, adminReject, adminCancel };
