...
function toResponseShape(row) {
  ...
  return {
    ...
    ccpAccountHolder: row.ccp_account_holder || '',
    ccpNumber: row.ccp_number || '',
  };
}
...
adminRouter.put(
  ...
  [
    ...
    body('ccpAccountHolder').optional({ nullable: true }).isString().trim().isLength({ max: 200 }),
    body('ccpNumber').optional({ nullable: true }).isString().trim().isLength({ max: 100 }),
  ],
  runValidation,
  async (req, res, next) => {
    try {
