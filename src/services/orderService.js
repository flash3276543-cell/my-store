...
/**
 * Admin: cancels an order regardless of its current payment status —
 * covers cases Reject doesn't (e.g. cancelling an already-completed
 * order). Only changes order_status; does not touch payment_status,
 * since undoing a verified payment is a separate (out of scope) refund
 * concern, not a plain cancellation.
 */
async function cancelOrder(orderId) {
  ...
  if (!rows[0]) {
    throw new OrderError('ORDER_ALREADY_CANCELLED', 'Order is already cancelled or was not found.', 409);
  }
  return { orderId };
}
