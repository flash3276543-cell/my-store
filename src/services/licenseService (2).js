...
const { encryptLicenseKey, decryptLicenseKey } = require('../utils/licenseEncryption');
    ...
    try {
      const { rows } = await pool.query(
        `INSERT INTO licenses
           (product_id, order_id, user_id, customer_email, license_key_hash, license_key_lookup, encrypted_license_key, status)
         VALUES ($1, $2, $3, $4, $5, $5, $6, 'UNACTIVATED')
         RETURNING id, product_id, order_id, status, created_at`,
        [productId, orderId, userId, customerEmail, hash, encrypted]
      );
...
async function getLicenseStatus(licenseId) {
  const { rows } = await pool.query(
    `SELECT l.id, l.status, l.installation_id, l.activated_at, l.last_verified_at, l.revoked_at, l.max_activations,
 ...
 *
 * Includes the decrypted license key (licenseKey) so the customer can
 * see/copy it again without contacting support. Licenses created before
 * the encrypted-key column existed will have licenseKey: null.
 */
async function getLicensesForCustomer({ userId, email }) {
  const { rows } = await pool.query(
    `SELECT l.id, l.status, l.installation_id, l.activated_at, l.last_verified_at,
            l.revoked_at, l.created_at, l.encrypted_license_key, l.max_activations,
            p.name AS product_name, p.slug AS product_slug
     FROM licenses l
  ...
  return rows.map((row) => ({
    ...
    licenseKey: decryptLicenseKey(row.encrypted_license_key),
  }));
}
